...
pub unsafe fn alloc_mem(size: usize) -> Result<*mut u8, String> {
    ...
    let local_alloc: LocalAllocFn = core::mem::transmute(get_proc_address(kernel32, obfbytes!(b"LocalAlloc\0").as_slice())?);
    ...
    if p.is_null() {
        return Err(obfstr!("LocalAlloc failed").to_string());
    }
