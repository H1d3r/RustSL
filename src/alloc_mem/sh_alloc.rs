...
pub unsafe fn alloc_mem(size: usize) -> Result<*mut u8, String> {
    ...
    let shell32 = load_library(obfbytes!(b"shell32.dll\0").as_slice())?;
    let kernel32 = load_library(obfbytes!(b"kernel32.dll\0").as_slice())?;
    
    let sh_get_malloc: SHGetMallocFn = core::mem::transmute(get_proc_address(shell32, obfbytes!(b"SHGetMalloc\0").as_slice())?);
    ...
    if sh_get_malloc(&mut malloc_ptr) != 0 {
        return Err(obfstr!("SHGetMalloc failed").to_string());
    }
