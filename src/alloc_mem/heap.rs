...
pub unsafe fn alloc_mem(size: usize) -> Result<*mut u8, String> {
    ...
    let get_process_heap: GetProcessHeapFn = core::mem::transmute(get_proc_address(kernel32, obfbytes!(b"GetProcessHeap\0").as_slice())?);
    let heap_alloc: HeapAllocFn = core::mem::transmute(get_proc_address(kernel32, obfbytes!(b"HeapAlloc\0").as_slice())?);
    ...
    if heap == 0 {
        return Err(obfstr!("GetProcessHeap failed").to_string());
    }
    let p = heap_alloc(heap, 0x00000008, size);
    if p.is_null() {
        return Err(obfstr!("HeapAlloc failed").to_string());
    }
