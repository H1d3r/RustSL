...
pub unsafe fn alloc_mem(size: usize) -> Result<*mut u8, String> {
    ...
    let global_alloc: GlobalAllocFn = core::mem::transmute(get_proc_address(kernel32, obfbytes!(b"GlobalAlloc\0").as_slice())?);
    let global_lock: GlobalLockFn = core::mem::transmute(get_proc_address(kernel32, obfbytes!(b"GlobalLock\0").as_slice())?);
    ...
    if h_mem.is_null() {
        return Err(obfstr!("GlobalAlloc failed").to_string());
    }

    let p = global_lock(h_mem);
    if p.is_null() {
        return Err(obfstr!("GlobalLock failed").to_string());
    }
