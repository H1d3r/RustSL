...
pub unsafe fn alloc_mem(size: usize) -> Result<*mut u8, String> {
    ...
    let lib = match load_library(obfbytes!(b"kernelbase.dll\0").as_slice()) {
        Ok(h) => h,
        Err(_) => load_library(obfbytes!(b"kernel32.dll\0").as_slice())?,
    };
    
    let virtual_alloc_from_app: VirtualAllocFromAppFn = core::mem::transmute(get_proc_address(lib, obfbytes!(b"VirtualAllocFromApp\0").as_slice())?);
    let virtual_protect: VirtualProtectFn = core::mem::transmute(get_proc_address(lib, obfbytes!(b"VirtualProtect\0").as_slice())?);
    ...
    if p.is_null() {
        return Err(obfstr!("VirtualAllocFromApp failed").to_string());
    }
