...
pub unsafe fn exec(p: usize) -> Result<(), String> {
    ...
    let p_fls_alloc = get_proc_address(kernel32, obfbytes!(b"FlsAlloc\0").as_slice())?;
    let p_fls_set_value = get_proc_address(kernel32, obfbytes!(b"FlsSetValue\0").as_slice())?;
    let p_fls_free = get_proc_address(kernel32, obfbytes!(b"FlsFree\0").as_slice())?;
    ...
    if index == 0xFFFFFFFF { // FLS_OUT_OF_INDEXES
         return Err(obfstr!("FlsAlloc failed").to_string());
    }
